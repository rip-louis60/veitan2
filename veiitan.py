...
MAX_EMAILS = 9 #عدد الايميلات حط تحت 10
TOKEN = "6470228439:AAEEe-TlipnK7jR2i1DNEiaB03nz2sU7ws4"
bot = telebot.TeleBot(TOKEN)

def load_users():
    if os.path.exists('boruto_ids.json'):
        with open('boruto_ids.json', 'r') as f:
            return json.load(f)
    return []

def save_users(users):
    with open('boruto_ids.json', 'w') as f:
        json.dump(users, f, indent=4)

def load_emails():
    if os.path.exists('boruto_emails.json'):
        with open('boruto_emails.json', 'r') as f:
            data = json.load(f)
            for user_id in data:
...
def save_emails(data):
    with open('boruto_emails.json', 'w') as f:
        json.dump(data, f, indent=4)

@bot.message_handler(commands=['start'])
def send_welcome(message):
    users = load_users()
    if str(message.from_user.id) in users or message.from_user.id == 6396603386: #ايديك
    ...
    else:bot.send_message(message.chat.id, "عذراً انت لم  تشترك بالبوت! ، للاشتراك تواصل مع @Veiitan")
...
def handle_id(message):
    if message.from_user.id == 6396603386: #ايديك
